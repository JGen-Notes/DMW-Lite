...
public class LangTypeComputer extends XbaseWithAnnotationsTypeComputer {
  protected void _computeTypes(final YCreateStatement expression, final ITypeComputationState state) {
    final Consumer<XExpression> _function = (XExpression element) -> {
      state.withNonVoidExpectation().computeTypes(element);
    };
    expression.getSetExpressions().forEach(_function);
    final Consumer<XExpression> _function_1 = (XExpression element) -> {
      state.withNonVoidExpectation().computeTypes(element);
    };
    expression.getSuccessExpressions().forEach(_function_1);
    final Consumer<XExpression> _function_2 = (XExpression element) -> {
      state.withNonVoidExpectation().computeTypes(element);
    };
    expression.getAlreadyExistExpressions().forEach(_function_2);
    state.acceptActualType(this.getPrimitiveVoid(state));
  }
  
  protected void _computeTypes(final YReadStatement expression, final ITypeComputationState state) {
    XExpression _whereclause = expression.getWhereclause();
    boolean _tripleNotEquals = (_whereclause != null);
    if (_tripleNotEquals) {
      final ITypeComputationState whereExpectation = state.withExpectation(this.getRawTypeForName(Boolean.TYPE, state));
      whereExpectation.computeTypes(expression.getWhereclause());
    }
    state.acceptActualType(this.getPrimitiveVoid(state));
  }
  
  public void computeTypes(final XExpression expression, final ITypeComputationState state) {
    if (expression instanceof YCreateStatement) {
      _computeTypes((YCreateStatement)expression, state);
      return;
    } else if (expression instanceof YReadStatement) {
      _computeTypes((YReadStatement)expression, state);
      return;
